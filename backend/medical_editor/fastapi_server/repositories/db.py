...
@router.put("/api/symptom/{id}", response_model=Symptom, response_description="updating an existing symptom")
async def update_symptom(id: str, req: UpdateSymptom = Body(...)):
    symptom = {k:v for k,v in req.dict().items() if v is not None} 
    update_symptom_data = await change_symptom(id,symptom)
    if update_symptom_data:
        return update_symptom_data
    raise HTTPException(404, f"Error occurred when updating symptom data")

@router.delete("/api/symptom/{id}", response_description="deleting an exisiting symptom")
async def delete_symptom(id):
    response = await remove_symptom(id)
    if response:
        return f"Successfully deleted {id}"
